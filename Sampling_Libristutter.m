filepath='C:\Users\reban\Documents\Speech Stuttering\Dataset\Libristutter\Annotation\';'FileExtension';'.csv';
ds=tabularTextDatastore(filepath);
filepath='C:\Users\reban\Documents\Speech Stuttering\Dataset\Libristutter\Audio\';'FileExtension';'.flac';
...
for i=80416:spec_num
    ...
    filename=strcat('Executable_Libristutter\Spectrogram_',num2str(i,formatspec),'.jpg');
...
ylabel=zeros(labelperfile,length(ds.Files));
 for i=1:length(ds.Files)
    ...
    Current=[round((c.Var2).*min_fs),round((c.Var3).*min_fs),double(c.Var4)];
 ...
 xlswrite('audio_label_Libristutter.xlsx',labels);